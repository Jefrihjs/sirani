...
{
    public function index(Request $request) // 1. Tambahkan Request di sini
    {
        $user = auth()->user();
        $userId = $user->id;
        $tahun = date('Y');

        // 2. Ambil parameter bulan dari filter, default bulan sekarang
        $bulan = $request->input('bulan', date('n'));

        // ================= KPI (Ganti now()->month dengan $bulan) =================
        ...
        // Status logic sesuai dengan class Tailwind baru
        if ($persenKPI >= 100) {
            $status = 'Tercapai';
        ...
        // 3. Pastikan 'bulan', 'user', dan 'tahun' dikirim ke View
