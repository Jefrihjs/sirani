                        ...
                        <td class="px-8 py-6">
                            <div class="flex items-center justify-end space-x-3">
                                @if($user->id !== auth()->id())
                                    ...
                                    class="flex items-center justify-center w-10 h-10 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-2xl transition-all shadow-sm active:scale-90"
                                    title="Edit User">
                                    ...
                                    {{-- Tombol HAPUS --}}
                                    <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline-flex m-0 form-hapus">
                                        @csrf 
                                        @method('DELETE')
                                        <button type="button" 
                                                onclick="konfirmasiHapus(this, event, '{{ $user->name }}')" 
                                ...
                                @else
                                    <span class="px-4 py-2 bg-slate-100 text-slate-400 text-[9px] font-black uppercase rounded-xl tracking-widest">Akun Anda</span>
                                @endif
                            </div>
                        </td>
                    </tr>
...
<script>
    function konfirmasiHapus(button, event, namaUser) {
        // Hentikan submit otomatis
        event.preventDefault();
        const form = button.closest('.form-hapus');

        Swal.fire({
            title: 'HAPUS PENGGUNA?',
            html: `Apakah Anda yakin ingin menghapus <b>${namaUser}</b>?<br><span class="text-[11px] text-slate-400">Seluruh data aktivitas user ini akan ikut terhapus.</span>`,
            icon: 'warning',
