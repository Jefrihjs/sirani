<aside id="sidebar" 
    x-data="{ 
        ...
        toggle() {
            ...
            {{-- Sinyal sakti supaya file app.blade.php ikut geser margin-nya --}}
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar relative z-10 overflow-x-hidden">
        ...
        <div class="p-4 mb-4 border-b border-white/5 bg-slate-950/30">
            <div class="flex" :class="isCollapsed ? 'flex-col items-center' : 'items-center justify-between mb-4'">
                ...
                {{-- TOMBOL PANAH TOGGLE --}}
                <button @click="toggle()" 
                    :class="isCollapsed ? 'w-full py-3' : 'w-10 h-10'"
                    class="flex items-center justify-center bg-white/5 hover:bg-blue-600 rounded-xl transition-all text-white shadow-inner">
                    <svg :class="isCollapsed ? 'rotate-180' : 'rotate-0'" 
                        class="w-5 h-5 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3">
        ...
        <nav class="px-3 space-y-1 mt-0">
            ...
            <div class="relative group/menu" x-data="{ open: false }">
                ...
                {{-- Dropdown Melayang (Saat Collapsed) --}}
                <div x-show="isCollapsed && open" @click.away="open = false" x-transition 
            ...
            @auth
                ...
                <div class="pt-4 pb-1 border-t border-white/5 mt-2 flex justify-center">
                    <span x-show="!isCollapsed" class="px-4 text-[8px] font-black text-slate-600 uppercase tracking-[0.2em] w-full text-left">
                        Otoritas Admin
                    </span>
                </div>
                
                <a href="{{ route('admin.users.index') }}" class="flex items-center space-x-3 px-4 py-2.5 rounded-xl transition-all {{ request()->routeIs('admin.users.*') ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}">
                    <span class="w-5 h-5 shrink-0">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    </span>
                    <span x-show="!isCollapsed" class="font-black text-[11px] uppercase tracking-[0.1em] whitespace-nowrap">Manajemen User</span>
                </a>
                @endif
            @endauth
        </nav>
    ...
    <div class="p-4 border-t border-white/5 bg-slate-950/50 backdrop-blur-md">
        ...
        <form method="POST" action="{{ route('logout') }}" class="px-2">
            ...
            <button type="submit" class="w-full flex items-center justify-center space-x-2 py-3 bg-rose-500/10 hover:bg-rose-600 text-rose-500 hover:text-white rounded-xl transition-all font-black text-[9px] uppercase tracking-[0.2em]">
                ...
                <span x-show="!isCollapsed" class="whitespace-nowrap">Logout</span>
            </button>
        </form>
    </div>
</aside>