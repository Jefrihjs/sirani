                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-indigo-50/30 transition-all duration-300 group">
                        ...
                        <td class="px-8 py-7 text-right">
                            <div class="flex items-center justify-end space-x-3 opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-0 translate-x-4">
                                {{-- EDIT --}}
                                <a href="{{ route('master-kegiatan.edit', $row->id) }}" 
                                   class="p-3 bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-2xl transition-all shadow-sm active:scale-90"
                                   title="Edit Kegiatan">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                </a>

                                {{-- HAPUS --}}
                                <form action="{{ route('master-kegiatan.destroy', $row->id) }}" method="POST" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" 
                                            class="p-3 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-2xl transition-all shadow-sm active:scale-90"
                                            onclick="return confirm('Hapus master kegiatan ini secara permanen?')">
